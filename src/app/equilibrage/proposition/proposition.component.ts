...
export class PropositionComponent implements OnInit {
  ...
  refreshList_articles_gen() {
    ...
    this.articleGenList = $('#datatable4').DataTable({
      data: this.articles_gen,
      info: false,
      columns: [
  ...
  refreshList_etabs() {
    if (this.crit == "seul_emet") {
      this.select_style = "single"
      this.button_all = []
    }
    if (this.tableEtabs) {
  ...
  Filtrer(pg?: any): void {
    if (this.crit != 'exist') {
      const data = {
        ...
        code_fournisseur: this.cf.join(','),
        fam1: this.m,
        fam2: this.c.join(','),
        code_couleur: this.code_couleur,
        date_injection: this.date_injection,
        fournisseur_principale: this.fp,
        filter_material: this.filter_material.join(','),
        filter_groupe: this.filter_groupe.join(','),
        filter_sous_fam: this.filter_sous_fam.join(','),
        filter_collection: this.filter_collection.join(','),
        filter_section: this.filter_section.join(','),
        filter_marque: this.filter_marque.join(','),
        filter_theme: this.filter_theme.join(','),
      ...
      this.articleService.getArticlesMultipleParams(data, this.page_number.toString())
        .subscribe({
          ...
          complete: () => {
            this.refreshList_articles()
            if ((this.cag != '') || (this.cb != '') || (this.cf.length > 0) || (this.m != '') || (this.c.length > 0) || (this.sc != '') || (this.fp != '') || (this.date_injection != '') || (this.code_couleur != '') || (this.filter_theme.length > 0) || (this.filter_marque.length > 0) || (this.filter_section.length > 0) || (this.filter_collection.length > 0) || (this.filter_sous_fam.length > 0) || (this.filter_groupe.length > 0) || (this.filter_material.length > 0)) {
            ...
            } else { this.filter_clicked = false }
          }
        });
    } else {
      const data = {
    ...
    this.etabService.getEtabssMultipleParams(data, '1000')
      .subscribe({
        ...
        complete: () => {
          ...
          } else { this.filter_clicked2 = false }
        }
      });
  }
